...
const calculateFCFS = async (req, res) => {
    try {
        ...
        // Validate and normalize process data (priority defaults to 1 for FCFS)
        ...
        const result = await runCppProgram('fcfs', input);
        ...
        console.error('FCFS Controller Error:', error);
        res.status(500).json({ 
            error: 'Internal server error while calculating FCFS',
